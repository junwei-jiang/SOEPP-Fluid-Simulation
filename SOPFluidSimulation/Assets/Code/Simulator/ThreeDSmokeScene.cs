...
{
    [Serializable]
    public class ThreeDSmokeScene : MonoBehaviour
    {
        void Start()
        {
            m_Resolution = new Vector3Int(128, 512, 128);
            m_Origin = new Vector3(0, 0, 0);
            m_Spacing = new Vector3(5.0f / m_Resolution.x, 20.0f / m_Resolution.y, 5.0f / m_Resolution.z);
            ...
            m_FCVFieldVelField = new CFaceCenteredVectorField(m_Resolution, m_Origin, m_Spacing);
            ...
            m_EulerFluid.setThreeDSmokeEmitter(0, 100000, 1.0f, 1.0f, new Vector3(0, 0, 0));
            m_EulerFluid.setBuoyancy(true);
            ...
            m_FCVFieldVisualizer = new CFCVFieldVisualizer(m_FCVFieldVelField);
        ...
        void OnRenderObject()
        {
            ...
            //m_FCVFieldVelField.resize(m_EulerFluid.getVelocityField());
            //m_FCVFieldVisualizer.visualize();
        }
        ...
        public void __write2File(string path, CCellCenteredScalarField vOutputField1, CCellCenteredScalarField vOutputField2)
        {
            using (FileStream fs = new FileStream(path, FileMode.OpenOrCreate, FileAccess.Write))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    ...
                    {
                        if (FieldValue1[i] < 0.1f)
                        {
                            FieldValue1[i] = 0.0f;
                        }
                        if (FieldValue2[i] < 0.1f)
                        {
                            FieldValue2[i] = 0.0f;
                        }
        ...
        private CFaceCenteredVectorField m_FCVFieldVelField;
